-- =============================================================================
-- Ready2Hybrid — MULTI-PAY-7S
-- Migration: 0021_multi_provider_canonical_tx_neutralization.sql
--
-- Forward-only CREATE OR REPLACE of public.webhook_apply_payment_tx(p jsonb).
-- Neutralizes hardcoded Mercado Pago provider identity. Does NOT change
-- order/hold/registration/ticket/capacity/audit business rules from 0009.
--
-- NOT applied by this unit. Do not execute against InsForge/Main here.
-- Apply later requires Lane A webhook payload to send explicit provider.
--
-- Predecessor: 0009_fix_webhook_payment_verification_order.sql
...
AS $$
DECLARE
  v_contract_provider text := btrim(COALESCE(p->>'provider', ''));
  v_provider text;
  v_evidence_ref text;
  v_audit_actor text;
  ...
  v_rank_old integer;
BEGIN
  -- Contract provider is required. No implicit Mercado Pago default.
  IF v_contract_provider = '' THEN
    RETURN jsonb_build_object(
      'ok', false,
      'error_code', 'INVALID_REQUEST',
      'error_detail', 'MISSING_PROVIDER'
    );
  ELSIF v_contract_provider = 'MERCADO_PAGO' THEN
    v_provider := 'MERCADOPAGO';
    v_evidence_ref := 'mp_payment:';
    v_audit_actor := 'mp-webhook';
  ELSIF v_contract_provider = 'CLIP' THEN
    v_provider := 'CLIP';
    v_evidence_ref := 'clip_payment:';
    v_audit_actor := 'canonical-payment-tx';
  ELSIF v_contract_provider = 'OPENPAY' THEN
    v_provider := 'OPENPAY';
    v_evidence_ref := 'openpay_payment:';
    v_audit_actor := 'canonical-payment-tx';
  ELSE
    RETURN jsonb_build_object(
      'ok', false,
      'error_code', 'INVALID_REQUEST',
      'error_detail', 'UNSUPPORTED_PROVIDER'
    );
  END IF;
  ...
  v_evidence_ref := v_evidence_ref || v_provider_payment_id;
  ...
  IF v_payment_found THEN
    ...
    SET order_id = v_order.id,
        ...
        sanitized_evidence_ref = v_evidence_ref,
...
COMMENT ON FUNCTION public.webhook_apply_payment_tx(jsonb) IS
  'MULTI-PAY-7S: provider-neutral successor of 0009. Requires explicit contract provider (MERCADO_PAGO|CLIP|OPENPAY). Durable storage MERCADOPAGO|CLIP|OPENPAY. Same order/hold/registration/ticket effects as 0009. Clip/Openpay runtime remains disabled.';
