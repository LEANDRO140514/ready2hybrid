...
-- Purpose: Atomic TX-2 payment-effect helper for mp-webhook (IMPL-8).
-- Authority: Explicit Project Owner authorization for IMPL-8.
--
-- Limits:
...
COMMENT ON FUNCTION public.webhook_apply_payment_tx(jsonb) IS
  'IMPL-8 TX-2: apply verified Mercado Pago payment effects atomically; no tickets/QR/email delivery.';
