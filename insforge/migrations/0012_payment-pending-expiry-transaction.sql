...
-- Ready2Hybrid — IMPL-14A-3B: canonical PAYMENT_PENDING expiry transaction.
--
-- Authority:
--  · SPEC-040 v0.1.1 APPROVED (R001-R009, R012-R015, R023-R024, I002-I009)
--  · IMPL-14A-2 v0.2.0 PLAN / APPROVED, §11.2 hardened sequence
--  · Project Owner authorization 2026-07-28, decisions D-1 … D-6
--
-- Scope of this migration:
--  · one supporting index for deterministic candidate selection (D-3)
--  · expire_payment_pending_aggregate_tx  — one eligible aggregate, atomic
--  · expire_payment_pending_batch_tx      — deterministic bounded sweep
--  · expire_payment_pending_dry_run_tx    — STABLE inspection, zero writes
--  · REVOKE / GRANT and function comments
--
-- Deliberately absent:
--  · Edge Functions, InsForge schedules, cron, anti-overlap (IMPL-14A-3C)
--  · administrative recovery, refund tasks, refund execution (IMPL-14A-3D)
--  · any change to webhook_apply_payment_tx or 0009 (D-5)
--  · any change to checkout_start_tx, opportunistic expiry, teams (D-4)
--  · table/column creation, constraints, RLS, backfill, physical DELETE
--
-- Temporal contract (SPEC-040 R001/R003/R019, aligned with 0011):
--   aggregate: v_expiry_now := clock_timestamp() once, after the order lock.
--   batch:     v_run_now := clock_timestamp() once per run (selection only).
--   dry-run:   v_now := statement_timestamp() once per SQL statement
--              (STABLE-coherent; no locks, so no post-lock wait clock).
--   now() is transaction start time and is stale after waiting for a lock,
--   so it is never used for eligibility. The frontier is closed (<=), as
--   SPEC-040 states it. expires_at IS NULL is unknown expiry, not expiry:
--   it never expires anything (fail-closed).
--   The residual asymmetry with webhook_apply_payment_tx (0009 uses
--   expires_at < now()) stays OPEN by D-5 for a separate hardening unit.
-- =====================================================================

-- D-3: keyset support for (state, expires_at, id) candidate selection.
-- Additive only; idx_capacity_holds_product_state_expires (0002) is neither
-- recreated nor modified.
CREATE INDEX IF NOT EXISTS idx_orders_state_expires_at_id
  ON public.orders (state, expires_at, id);


-- =====================================================================
-- expire_payment_pending_aggregate_tx — SPEC-040-R008 atomic aggregate
-- =====================================================================
...
BEGIN
  ...
  LOOP
    ...
    v_hold_ids := v_hold_ids || v_hold.id;
    v_hold_states := v_hold_states || v_hold.state;
  ...
  LOOP
    ...
    v_reg_ids := v_reg_ids || v_reg.id;
    v_reg_states := v_reg_states || v_reg.state;
  ...
  FOR v_cred IN
    ...
    FOR UPDATE
  LOOP
    v_holder_ids := v_holder_ids || v_cred.id;
    v_holder_states := v_holder_states || v_cred.state;
  ...
  IF v_pay_approved > 0 THEN
    v_findings := v_findings || 'APPROVED_PAYMENT_ORDER_NOT_PAID';
  END IF;
  IF v_pay_terminal > 0 THEN
    v_findings := v_findings || 'PAYMENT_TERMINAL_MISMATCH';
  END IF;
  IF v_hold_total = 0 THEN
    v_findings := v_findings || 'HOLD_MISSING';
  ELSIF v_hold_active = 0 THEN
    v_findings := v_findings || 'HOLD_NO_ACTIVE';
  ELSIF v_hold_active > 1 THEN
    v_findings := v_findings || 'HOLD_MULTIPLE_ACTIVE';
  END IF;
  IF v_hold_active = 1 AND v_active_hold_expires IS NULL THEN
    v_findings := v_findings || 'HOLD_EXPIRY_UNKNOWN';
  ELSIF v_hold_active = 1 AND v_active_hold_expires > v_expiry_now THEN
    v_findings := v_findings || 'HOLD_NOT_TIME_EXPIRED';
  END IF;
  IF v_hold_converted > 0 OR v_access_artifacts > 0 THEN
    v_findings := v_findings || 'PARTIAL_CONVERSION';
  END IF;
  IF v_reg_total = 0 THEN
    v_findings := v_findings || 'REGISTRATIONS_MISSING';
  ELSIF v_reg_provisional <> v_reg_total THEN
    v_findings := v_findings || 'REGISTRATION_NON_PROVISIONAL';
  END IF;
  IF v_holder_active = 0 THEN
    v_findings := v_findings || 'ORDER_HOLDER_MISSING';
  ELSIF v_holder_active > 1 THEN
    v_findings := v_findings || 'ORDER_HOLDER_MULTIPLE';
  END IF;
  ...
  IF v_hold_active = 1
    ...
    v_findings := v_findings || 'EXPIRES_AT_DIVERGENCE';
  END IF;
...
-- expire_payment_pending_batch_tx — deterministic bounded sweep
-- =====================================================================
-- Transaction semantics, stated explicitly:
--  · the whole RPC is still a single PostgreSQL transaction;
--  · a crashed invocation rolls back the entire batch;
--  · per-item commit belongs to the IMPL-14A-3C Edge Function.
-- A controlled per-item error is isolated in its own subtransaction so it does
-- not invalidate the rest of the batch. Job anti-overlap is NOT implemented
-- here: it is IMPL-14A-3C work and OD-040-001 keeps it OPEN.
-- Rows locked by another worker are skipped by SKIP LOCKED at selection time,
-- so they never enter items[]; the per-item skipped_locked outcome can only
-- surface through a direct aggregate call.
-- =====================================================================
CREATE OR REPLACE FUNCTION public.expire_payment_pending_batch_tx(p jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = pg_catalog, public, pg_temp
...
DECLARE
  ...
  v_run_id text := COALESCE(NULLIF(p->>'run_id', ''), gen_random_uuid()::text);
  v_actor text := COALESCE(NULLIF(p->>'actor_ref', ''), 'system:expiry-reconciler');
  v_fingerprint text := NULLIF(p->>'idempotency_fingerprint', '');
  v_run_now timestamptz;
  v_candidate uuid;
  v_result jsonb;
  v_outcome text;
  v_items jsonb := '[]'::jsonb;
  ...
  v_order_not_found integer := 0;
  v_error integer := 0;
BEGIN
  -- One instant per run for candidate selection; each item re-reads its own
  -- canonical clock after its own lock inside the aggregate.
  v_run_now := clock_timestamp();
  ...
  FOR v_candidate IN
    ...
    WHERE state = 'PAYMENT_PENDING'
      ...
      AND expires_at <= v_run_now
    ORDER BY expires_at ASC, id ASC
    LIMIT v_limit
    FOR UPDATE SKIP LOCKED
  LOOP
    BEGIN
      v_result := public.expire_payment_pending_aggregate_tx(
        jsonb_build_object(
          'order_id', v_candidate,
          'correlation_id', v_run_id || ':' || v_candidate::text,
          'actor_ref', v_actor,
          'idempotency_fingerprint', v_fingerprint
        )
      );
      v_outcome := COALESCE(v_result->>'outcome', 'error');
    EXCEPTION
      WHEN OTHERS THEN
        -- SQLSTATE only: an error message could echo row content.
        v_outcome := 'error';
    ...
    ELSIF v_outcome = 'noop' THEN
      v_noop := v_noop + 1;
    ELSIF v_outcome = 'inconsistent' THEN
      v_inconsistent := v_inconsistent + 1;
    ELSIF v_outcome = 'skipped_locked' THEN
      v_skipped_locked := v_skipped_locked + 1;
    ELSIF v_outcome = 'order_not_found' THEN
      v_order_not_found := v_order_not_found + 1;
    ELSE
      v_error := v_error + 1;
    END IF;
  END LOOP;
...
-- =====================================================================
-- expire_payment_pending_dry_run_tx — SPEC-040-R013 inspection
-- =====================================================================
-- STABLE is the enforcement, not a label: PostgreSQL refuses to execute
-- INSERT / UPDATE / DELETE / SELECT FOR UPDATE inside a non-volatile function,
-- so zero durable mutation is guaranteed by the engine and not only by review.
-- Evaluation clock is statement_timestamp(): fixed for the whole SQL statement,
-- so the STABLE promise (same args → same result within one statement) holds.
-- clock_timestamp() is reserved for aggregate/batch paths that evaluate after
-- acquiring or waiting on locks. The assessment logic is intentionally
-- duplicated instead of factored into a helper: this migration is authorized
-- to create exactly three functions. It reports candidates and reasons and
-- must never look like an applied reconciliation: no activity_log, no outbox,
-- no refund task, no ticket, no timestamp repair, no locks.
-- =====================================================================
...
BEGIN
  ...
  LOOP
    ...
    IF r.expires_at IS NULL THEN
      ...
      v_findings := v_findings || 'NOT_ELIGIBLE_EXPIRY_UNKNOWN';
    ELSE
...
-- =====================================================================
-- Grants (D-6)
-- project_admin is the current repository integration role;
-- this grant does not claim least privilege;
-- OD-040-002 remains OPEN;
-- IMPL-14A-3C and production remain blocked.
-- =====================================================================
...
REVOKE ALL ON FUNCTION public.expire_payment_pending_batch_tx(jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.expire_payment_pending_batch_tx(jsonb) FROM anon;
REVOKE ALL ON FUNCTION public.expire_payment_pending_batch_tx(jsonb) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.expire_payment_pending_batch_tx(jsonb) TO project_admin;
...
COMMENT ON FUNCTION public.expire_payment_pending_aggregate_tx(jsonb) IS
  'IMPL-14A-3B: atomic natural expiry of one PAYMENT_PENDING checkout aggregate (SPEC-040 R001-R009, R012-R015, R023-R024, I002-I009). Locks orders first, then holds, registrations and ORDER_HOLDER credentials; canonical clock is clock_timestamp() after the lock; fail-closed to REQUIRES_REVIEW on inconsistency; zero tickets, zero payment mutation, zero refunds. Execute granted to project_admin only, which is not a least-privilege claim: OD-040-002 remains OPEN.';

COMMENT ON FUNCTION public.expire_payment_pending_batch_tx(jsonb) IS
  'IMPL-14A-3B: deterministic bounded sweep (keyset expires_at, id; default 25, hard cap 50; FOR UPDATE SKIP LOCKED) calling the aggregate per item in its own subtransaction. Still one PostgreSQL transaction: a crashed invocation rolls back the whole batch and per-item commit belongs to the IMPL-14A-3C Edge Function. No job anti-overlap here: OD-040-001 remains OPEN and IMPL-14A-3C is not authorized.';

COMMENT ON FUNCTION public.expire_payment_pending_dry_run_tx(jsonb) IS
  'IMPL-14A-3B: STABLE inspection of expiry candidates (SPEC-040 R013, AC009, AC017). Evaluation instant is one statement_timestamp() per SQL statement (STABLE-coherent; no locks). Aggregate/batch keep post-lock clock_timestamp(). PostgreSQL forbids data modification and row locking inside a non-volatile function, so this reports eligible / noop / inconsistent plus deferred PREFERENCE_PENDING_HOLD_PERSISTENCE holds without writing anything, and never presents itself as an applied reconciliation.';
