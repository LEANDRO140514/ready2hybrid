-- =====================================================================
-- 0023_buyer-contact-checkout-write.sql
-- Purpose: Add buyer_contacts.contact_consent_at and teach checkout_start_tx
--          to persist email/name/phone/consent for ticket delivery.
-- Predecessor: 0022_buyer-contact-fields.sql (email/name/phone columns)
-- Authority: Project Owner decisions PASO 2 — 2026-08-25
--
-- This migration:
--  - ADD COLUMN contact_consent_at timestamptz NULL
--  - CREATE OR REPLACE checkout_start_tx to INSERT contact PII
--  - Opt-in marketing/contact consent: request boolean buyer_contact_consent
--    maps to contact_consent_at = now() when true, else NULL (never default true)
--
-- Requires 0022 applied first (email, name, phone columns + email CHECK).
--
-- Explicitly out of scope:
--  · Apply to Main / sandbox (not authorized by this unit)
--  · Landing UI / RegistroPage
--  · selected_provider landing gap (pre-existing; separate unit)
--  · Email delivery / Resend / outbox worker
--  · Hardening email to NOT NULL
--  · New RLS policies
--
-- TRANSACTION CONTROL:
...
COMMENT ON COLUMN public.buyer_contacts.contact_consent_at IS
  'PII-adjacent: timestamp when buyer opted in to contact/marketing (noticias, novedades, ajustes, próximos eventos). NULL = no opt-in. Distinct from sports participation waiver (waiver_acceptances).';

COMMENT ON TABLE public.buyer_contacts IS
  'SPEC-032 BuyerContact; not automatically payer/participant/access holder. Personal contact fields (email, name, phone, contact_consent_at) are PII used for ticket delivery and operational/marketing contact. Inventory closed by Owner 2026-08-25; email remains NULLABLE at column level until a later successor; checkout_start_tx requires email/name at write time.';

-- ---------------------------------------------------------------------
-- checkout_start_tx successor (body from 0020 + buyer contact write)
-- ---------------------------------------------------------------------
...
COMMENT ON FUNCTION public.checkout_start_tx(jsonb) IS
  'TX-1 checkout start; OD-001 spectator multi-quantity; v0.4 organizer sale_state SOLD_OUT (event/category/product, parent wins); cupo/holds are not commercial SOLD_OUT; buyer email/name required (CONTACT_REQUIRED); optional phone; contact_consent_at from buyer_contact_consent boolean.';
