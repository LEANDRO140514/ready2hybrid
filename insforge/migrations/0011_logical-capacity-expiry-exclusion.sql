-- =====================================================================
-- 0011_logical_capacity_expiry_exclusion.sql
-- Purpose: SPEC-040-I007 / SPEC-040-R004 — exclude time-expired ACTIVE holds
--          from availability / cupo calculations (logical guarantee only),
--          judged by a lock-aware canonical clock captured once post-lock.
-- Authority: IMPL-14A-3A authorized by Project Owner 2026-07-27
-- Governing: SPEC-040 v0.1.1 APPROVED; IMPL-14A-2 plan v0.2.0 PLAN / APPROVED
-- Baseline: e6c812b
--
-- Limits:
--  · CREATE OR REPLACE checkout_start_tx only (inventory predicate change).
--  · Does NOT persist order/hold EXPIRED, cancel registrations, or expire
--    ORDER_HOLDER (those belong to later IMPL-14A-3B+ units).
--  · Does NOT create opportunistic expiry writes.
--  · Does NOT recreate the existing capacity_holds (product_id, state, expires_at)
--    index from migration 0002.
--  · Does NOT edit prior migration files on disk.
--  · Does NOT add NOT NULL constraints, backfill, or repair historical rows.
--  · Does NOT apply remotely in this unit (local contract only).
-- =====================================================================
...
COMMENT ON FUNCTION public.checkout_start_tx(jsonb) IS
  'TX-1 checkout start; OD-001 spectator multi-quantity; SPEC-040-R004 excludes time-expired ACTIVE holds from cupo.';
