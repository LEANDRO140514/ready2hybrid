-- =====================================================================
-- 0028_affiliates.sql
-- Purpose: Optional affiliate attribution on checkout orders; commission
--          reporting view over PAID orders that still have a valid ticket.
-- Predecessor: 0025_checkout-team-roster-capture.sql (live checkout_start_tx)
--              0027_ticket-revoke-tx.sql (revocation already live on Main)
--
-- Debt (OD-007): revocación parcial de una orden multi-ticket sigue
-- contando la orden completa; se resuelve con OD-007.
--
-- This migration does NOT apply itself to Main. Owner authorizes apply.
-- Does NOT modify mp-webhook, payments tables, or ticket issuance RPCs.
--
-- TRANSACTION CONTROL:
...
COMMENT ON TABLE public.affiliates IS
  'Optional referral partners. Checkout stores affiliates.code on orders when the submitted code is active.';

-- RLS pattern copied from event_category_sale_states create-time DCL
-- (0020_v04-commercial-authority-successor.sql lines 89-94) and products
-- FORCE RLS (0003 lines 46-47) + anon/authenticated REVOKE (0015 line 136).
...
REVOKE SELECT, INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER
  ON TABLE public.affiliates FROM anon, authenticated;

GRANT SELECT ON public.affiliates TO project_admin;

ALTER TABLE public.orders
  ADD COLUMN affiliate_code text NULL REFERENCES public.affiliates(code);

CREATE INDEX orders_affiliate_code_idx
  ON public.orders (affiliate_code)
  WHERE affiliate_code IS NOT NULL;

CREATE VIEW public.v_affiliate_sales AS
WITH paid AS (
  SELECT o.id, o.affiliate_code, o.total_cents
...
valid_tickets AS (
  SELECT o.affiliate_code, count(*) AS tickets_valid
  FROM public.orders o
...
LEFT JOIN valid_tickets vt ON vt.affiliate_code = a.code
GROUP BY a.code, a.name, a.active, a.commission_bps, vt.tickets_valid
ORDER BY commission_cents DESC;

GRANT SELECT ON public.v_affiliate_sales TO project_admin;
