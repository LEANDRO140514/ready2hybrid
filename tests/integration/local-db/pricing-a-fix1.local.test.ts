 ...
 * 0028 is the live checkout_start_tx successor (optional affiliate_code).
 *
 ...
 * - tests/unit/expiry/logical-capacity-exclusion.test.ts (0028 TX contract)
 */
...
const MIGRATION_0028 = '0028_affiliates.sql'
...
describe('Pricing-A FIX-1 → v0.4 commercial successor static guards', () => {
  ...
  it('0020 introduced organizer sale_state SOLD_OUT; 0028 is live checkout_start_tx', () => {
    ...
    const sql28 = fs.readFileSync(migrationPath(MIGRATION_0028), 'utf8')
    const code28 = stripSqlComments(sql28)
    expect(sql28).toContain('CREATE OR REPLACE FUNCTION public.checkout_start_tx(p jsonb)')
    expect(code28).toMatch(/sale_state IS NOT DISTINCT FROM 'SOLD_OUT'/)
    expect(code28).toContain("error_code', 'SOLD_OUT'")
    expect(code28).not.toMatch(/INTO v_active_holds/)
    expect(code28).not.toMatch(
      /IF v_active_holds\s*\+\s*v_units\s*>\s*v_product\.cupo/i,
    )
    expect(code28).toMatch(/cupo\/holds are not commercial SOLD_OUT/i)
    expect(code28).toContain('CONTACT_REQUIRED')
    expect(code28).toContain('contact_consent_at')
    expect(code28).toContain('v_affiliate_code')
    expect(code28).not.toMatch(/BYPASSRLS/i)
    expect(sql28).not.toMatch(/^\s*BEGIN;|^\s*COMMIT;/m)
  })
})
